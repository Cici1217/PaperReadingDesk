...
const homeTranslations = {
  ...
  ja: {
    ...
    codexFormTitle: "ローカルCodexを設定", codexFormLead: "Codexをインストールしてログインし、設定を保存してテストします。", claudeFormTitle: "Claude Codeを設定", claudeFormLead: "未インストールでも保存できます。テスト時にインストールまたはログインのエラーを明示します。", querySummary: "Codexのログインと確認", query1: "codex login statusでログインを確認します。", query2: "codexを起動し、/modelでモデルと推論強度を確認します。", query3: "/statusで現在の選択を確認します。", claudeGuideTitle: "Claude Codeのインストールとログイン", claudeQuery1: "初回起動時にブラウザーが開き、Claudeアカウントでログインします。", claudeQuery2: "ログイン画面が出ない場合はClaude Codeで/loginを入力します。", claudeQuery3: "/statusで確認します。ブラウザーが開かない場合はcでURLをコピーします。", claudeSecretNote: "コマンド、モデル、テスト状態だけを保存し、認証情報は保存しません。", codexCommandLabel: "Codexコマンド", claudeCommandLabel: "Claude Codeコマンド", codexModelPlaceholder: "/modelで確認", claudeModelPlaceholder: "sonnet、opus、または完全なモデル名", configuredHint: "テスト成功後は現在のAIに自動設定され、確認済み設定は手動でも切り替えられます。", save: "設定を保存", test: "設定をテスト", useProvider: "現在のAIに設定", statusChecking: "読み込み中…", statusReady: "設定済み", statusMissing: "未設定", authChatgpt: "ChatGPTアカウント", authApiKey: "API Key", authToken: "Access Token", authClaude: "Claudeアカウント", authAuthenticated: "認証済み", authNone: "未確認", saving: "保存中…", testing: "テスト中…", savedNotice: "保存しました。続けてテストしてください", testedNotice: "設定テストに成功し、現在のAIに設定しました", activatedNotice: "現在のAIを切り替えました"
  },
  en: {
...
async function test() {
  ...
  try {
    ...
    await SelfPageAPI.request(`/api/${selectedProvider}/test`, {method: "POST", body: {}});
    aiStatus = await SelfPageAPI.request("/api/ai/status"); loadInputs(); render(); notify(th("testedNotice"));
